...
MODEL_DIR = "/data/hdd1/users/kmparmp/experiment2/job/train/random/models/loger/random/final_model/"
FINAL_MODEL_PATH = os.path.join(MODEL_DIR, "20250521_093859_final_1.pkl")
FINAL_METADATA_PATH = os.path.join(MODEL_DIR, "20250521_093859_final_1_metadata.json")
FINAL_BASELINE_PATH = os.path.join(MODEL_DIR, "20250521_093859_final_1_baseline.pkl")
...
def test_loger_model(model, workload_path, verbose=False, faiss_index_path="plan_embeddings.faiss"):
    ...
    if os.path.exists(faiss_index_path):
        ...
        # Get embedding dim by making a test pass
        test_sql = next(iter(load_sql_files(workload_path, verbose=False)))
        _, _, _, _, test_embedding = model.beam_plan(test_sql.to(device), bushy=False, judge=False, return_prediction=True, return_embedding=True)
    ...
    with torch.no_grad():
        for sql in tqdm(sql_objects, desc="Testing queries"):
            try:
                ...
                plan, _, _, predicted_latency, final_embedding = model.beam_plan(
                ...
                final_embedding = final_embedding.cpu().numpy().astype('float32')
                faiss_index.add(final_embedding.reshape(1, -1))  # Add single embedding
    ...
    print(f"Saved {faiss_index.ntotal} embeddings to {faiss_index_path}")
